...
{
	char* a1 = ((Airport*)airport1)->code;
	char* a2 = ((Airport*)airport2)->code;
...
int readAirportFromTextFile(FILE* file, Airport* airport)
{
	...
	if (!airport->name)
		errorFlag = 0;
	if (readStringFromTextFile(file, airport->name) == 0)
		errorFlag = 0;
	airport->country = (char*)malloc(sizeof(char)*MAX_STR_LEN);
	if(!airport->country)
		errorFlag = 0;
	if (readStringFromTextFile(file, airport->country) == 0)
		errorFlag = 0;
	if (readStringFromTextFile(file, airport->code) == 0)
		errorFlag = 0;
...
{
	char* enter = "\n";
	Airport* pAp = (Airport*)airport;
	fwrite(pAp->name, sizeof(char), strlen(pAp->name), file);
	fwrite(enter, sizeof(char), 1, file);
	fwrite(pAp->country, sizeof(char), strlen(pAp->country), file);
	fwrite(enter, sizeof(char), 1, file);
	fwrite(pAp->code, sizeof(char), CODE_LENGTH, file);
	fwrite(enter, sizeof(char), 1, file);
}